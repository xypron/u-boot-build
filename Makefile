...
build-ipxe:
	...
	cd ipxe/src && make bin-arm-efi/snp.efi -j$(NPROC) \
...
build:
	...
	cd patch && (git rebase)
	test -f ipxe/src/bin-arm-efi/snp.efi || make build-ipxe
	cp ipxe/src/bin-arm-efi/snp.efi tftp/snp-arm.efi
...
check:
	...
	-device e1000,netdev=eth0 \
	-drive if=none,file=arm.img,format=raw,id=mydisk \
	-device ich9-ahci,id=ahci -device ide-drive,drive=mydisk,bus=ahci.0
